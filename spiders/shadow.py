...
class ShadowSpider(threading.Thread):
    ...
    def __init__(self, book_link, type, debug, lock):
    ...
    def run(self):
        if self.data_type is DataType.DETAIL:
            try:
                # self.get_single_book_details(self.is_debug)
                print(self.name + ' Ended.')
            except Exception as e:
                print(self.book_link.link + '\n' + e)

    def get_single_book_details(self, book_link, data_type):
        print(self.name + ' : Processing ' + book_link.link)
        ...
        # if write_to_file:
        datahandler.save_to_file(bookfilepath + '/test', 'BookDetails_test', complete_data)
        datahandler.save_to_file(bookfilepath + '/test', 'AlsoLikes_test', also_likes_txt)
        datahandler.save_to_file(bookfilepath + '/test', 'Also_E_Likes_test', also_e_likes)
        datahandler.save_to_file(bookfilepath + '/test', 'Introduction_test', intro)
        self.lock.release()
        ...
        print('===' + self.name + '::' + 'Y' + '::' + book_link.link)

    def go(self, task):
        print('Test')
        if task.data_type is DataType.DETAIL:
            try:
                self.get_single_book_details(task.book_link)
                print(self.name + ' Ended.')
            except Exception as e:
                print(task.book_link.link + '\n' + e)
...
class SpiderTask:
    ...
    def __init__(self, param_book_link, param_data_type):
