...
def get_single_book_details(quoted_url, write_to_file):
    ...
    if score_details.__len__() > 2:
        number = re.search(r'[\d]+', score_details[2], flags=0).group()
    else:
        number = '0'
    if score_details.__len__() > 3:
        star5 = re.search(r'[\d]+.[\d]+', score_details[4], flags=0).group()
    else:
        star5 = '0'
    if score_details.__len__() > 5:
        star4 = re.search(r'[\d]+.[\d]+', score_details[6], flags=0).group()
    else:
        star4 = '0'
    if score_details.__len__() > 7:
        star3 = re.search(r'[\d]+.[\d]+', score_details[8], flags=0).group()
    else:
        star3 = '0'
    if score_details.__len__() > 9:
        star2 = re.search(r'[\d]+.[\d]+', score_details[10], flags=0).group()
    else:
        star2 = '0'
    if score_details.__len__() > 11:
        star1 = re.search(r'[\d]+.[\d]+', score_details[12], flags=0).group()
    else:
        star1 = '0'
    ...
    intro = config.dataSplitter + book_name + config.dataSplitter + isbn + config.dataSplitter + ('NA' if short_intro is None else short_intro.get_text().strip()) + '\n'
    print(intro)
